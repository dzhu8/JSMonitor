    ...
    if paths:
        for alias_pattern, target_patterns_in_tsconfig in paths.items():
            ...
            for target_path_pattern in target_patterns_in_tsconfig:
                ...
                abs_target_dir = os.path.normpath(os.path.join(abs_base_url, target_dir_rel_to_baseurl))
...
def format_single_import_path(original_module_path, current_file_abs_dir, path_aliases_map, abs_base_url):
    ...
    if not original_module_path.startswith('.'): # and not os.path.isabs(original_module_path):
        return original_module_path

    # Resolve the original import path to an absolute path
    if os.path.isabs(original_module_path): # Should generally not happen for typical project imports
        abs_imported_item_path = os.path.normpath(original_module_path)
    else:
        abs_imported_item_path = os.path.normpath(os.path.join(current_file_abs_dir, original_module_path))
    ...
    for alias_prefix_str, abs_target_dirs_for_alias in path_aliases_map.items():
        for abs_target_dir in abs_target_dirs_for_alias:
            ...
            target_dir_path_for_match = os.path.join(abs_target_dir, "") # Adds os.sep if not present at end
            ...
            if abs_imported_item_path.startswith(target_dir_path_for_match):
                ...
                relative_suffix = abs_imported_item_path[len(target_dir_path_for_match):]
                relative_suffix = relative_suffix.replace(os.sep, '/') # Standardize to forward slashes
...
def process_file_content(content, file_abs_dir, path_aliases_map, abs_base_url):
    ...
    def replace_import_path_match(match_obj):
        ...
        new_path = format_single_import_path(original_path, file_abs_dir, path_aliases_map, abs_base_url)
        ...
        if new_path != original_path:
            ...
            return match_obj.group(0).replace(quote_char + original_path + quote_char, 
...
def main():
    ...
    if not path_aliases:
        print("No path aliases found or tsconfig/jsconfig.json processing failed. Exiting.")
        return
    ...
    for file_path in files_to_scan:
        ...
        file_abs_dir = os.path.dirname(file_path) # file_path is already absolute from glob
        modified_content = process_file_content(original_content, file_abs_dir, path_aliases, abs_base_url)
